...
from app import db
from application.podcasts.models import Podcast, PopularPodcast
...
def delete_podcast(podcast):
    ...
    db.session.commit()
    path = os.path.join(app.root_path, 'application', 'static', 'podcasts', podcast.audio_file)
    os.remove(path)
...
def save_cover(file):
    ...
    file_path = os.path.join(app.root_path, 'application/static/podcast_covers', new_filename)
...
def save_audio(file):
    ...
    _, f_ext = os.path.splitext(file.filename)
    new_filename = random_hex + f_ext
    file_path = os.path.join(app.root_path, 'application/static/podcasts', new_filename)
